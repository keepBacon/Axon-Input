...
7. “显示行为”新增 CPS 显示：开启后可绑定外接键盘按键、鼠标左键或鼠标右键，以 1 秒滑动窗口实时显示 cps: N；未绑定时显示 cps: --，支持自由拖动与独立透明度。
8. “显示行为”新增“导入字体”：支持 TTF / OTF，导入后复制到应用内部目录，原生按显、CPS、最近按键和手柄文本统一使用。
9. 优化输入灵敏度倍率：1–200% 使用 1% 精度，200–500% 使用 5% 精度；新增恢复 100%；滑动倍率时只刷新灵敏度代理，不重建其他悬浮状态。
10. 灵敏度代理减少重复配置文件读取；100% 鼠标倍率直接透传，降低默认倍率下的计算开销。
...
- 输入显示全键盘：输入法打开时显示只读键盘，输入按键会高亮，输入法关闭后隐藏。

手柄兼容性：
- L2/R2 支持 ABS_BRAKE/ABS_GAS、ABS_Z/ABS_RZ 和数字 BTN_TL2/BTN_TR2。
- 双切扳机在模拟轴与数字微动模式之间切换时会合并状态。
- 支持静止值在最大端的反向扳机轴。
