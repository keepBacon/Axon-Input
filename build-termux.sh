...
# Keep the signing key outside the project folder so later Axon Input builds reuse it.
...
# 2) Resources
# Compile the complete Android resource tree, including PNG launcher assets.
# The previous XML-only loop skipped drawable-nodpi/ic_axon_symbol.png, causing
# aapt2 link to report drawable/ic_axon_symbol as missing.
aapt2 compile \
...
# 3) Build-time signature pin. It is generated from the same persistent key used to sign this APK.
...
# 6) Package DEX + JNI. extractNativeLibs=true, so zipalign is not required.
...
# 7) Sign with the same certificate pinned into BuildSignature.
apksigner sign \
    --ks "$KEYSTORE" \
...
# 8) Copy the signed APK to the phone Download directory.
# Prefer Termux shared-storage symlink; fall back to Android's canonical path.
DOWNLOAD_DIR=""
